...
impl<'r> Record<'r> {
    ...
    pub fn parse_client_appdata(bytes: &'r [u8]) -> Result<(Record<'r>, &'r [u8]), RecordError> {
        ...
        let (content, rest_next) = match hdr.content_type {
            ContentType::Alert => {
                let (c, r_next) = AlertMsg::client_parse(rest)?;
    ...
    pub fn parse_server<P: ServerRecordProcessor>(
        ...
        let (content, rest_next) = match hdr.content_type {
            ContentType::Handshake => {
                let (c, r_next) = HandshakeMsg::server_parse(prc, rest)?;
    ...
    pub fn parse_client<P: ClientHelloProcessor>(
        ...
        let (content, rest_next) = match hdr.content_type {
            ContentType::Handshake => {
                let (c, r_next) = HandshakeMsg::client_parse(prc, rest)?;
                (Content::Handshake(c), r_next)
            }
            ContentType::Alert => {
                let (c, r_next) = AlertMsg::client_parse(rest)?;
...
mod test {
    ...
    fn test_firefox_handshake_client_hello() {
        ...
        let (r, rest) =
            Record::parse_client(&mut tester, &data).expect("Test: Record parsing failure.");

...
use crate::{TlsServerCtxError, Rfc8446Error};
...
impl<C: TlsServerCtxConfig, Crypto: CryptoConfig, Rng: CryptoRng> CtxHandshakeProcessor for ServerHandshakeCtx<C, Crypto, Rng> {
    ...
    #[inline]
    fn spin_handshake<Li: TlsLeftIn, Lo: TlsLeftOut>(&mut self, li: &mut Li, lo: &mut Lo) -> Result<Option<HandshakeComplete>, Self::Error> {
        ...
        loop {
            ...
            println!("Consumed {consumed} Remaining len = {} out of initial {init_len}", remaining.len());
            ...
            match rec.content() {
                ...
                Content::ApplicationData => {
                    println!("ApplicationData {} ..  = {}", hex::encode(rec.header_as_bytes()), hex::encode(rec.as_bytes()));
                    ...
                    body[0..body_len]
                        .copy_from_slice(&full_payload[0..body_len]);
                    ...
                    use ytls_record::{WrappedRecord, WrappedMsgType};
                    ...
                    if let WrappedMsgType::Handshake( HandshakeMsg { msg: MsgType::ClientFinished(f), .. }) = r.msg() {
                ...
                Content::Handshake(content) => {
                    ...
                    match msg {
                        ...
                        MsgType::ClientHello(h) => {
                            ...
                            self.handshake_finished_server_key = Some(server_handshake_finished_key);
                            ...
                            self.handshake_client_iv = Some(Nonce12::from_ks_iv(&client_handshake_iv));
                            self.handshake_finished_client_key = Some(client_handshake_finished_key);
                            ...
                            let finish_handshake_hash = transcript_more.sha256_fork().sha256_finalize();
                            ...
                            self.application_server_iv = Some(Nonce12::from_ks_iv(&server_application_iv));
                            self.application_client_key = Some(client_application_key);
                            self.application_client_iv = Some(Nonce12::from_ks_iv(&client_application_iv));
                        }
                    }
                }
                Content::Alert(alert) => {

...
pub use hkdf::{Hkdf, GenericHkdf, hmac::Hmac};

impl Sha256Hkdf {
    pub fn sha256_hkdf_init() -> Self {
        Self {}
    }
    pub fn sha256_hkdf_from_prk(prk: &[u8]) -> Result<impl CryptoSha256HkdfGenProcessor, hkdf::InvalidPrkLength> {
        Ok(Sha256HkdfExtract { inner: Hkdf::<Sha256>::from_prk(prk)? })
    }
}

impl CryptoSha256HkdfExtractProcessor for Sha256Hkdf {
    fn hkdf_sha256_extract(&self, salt: Option<&[u8]>, ikm: &[u8]) -> ([u8; 32], impl CryptoSha256HkdfGenProcessor) {
